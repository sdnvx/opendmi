...
const dmi_entity_spec_t dmi_memory_device_spec =
{
    ...
    .handlers    = {
        .decode = (dmi_entity_decode_fn_t)dmi_memory_device_decode,
        .link   = (dmi_entity_link_fn_t)dmi_memory_device_link,
        .free   = (dmi_entity_free_fn_t)dmi_memory_device_free
    }
};
...
dmi_memory_device_t *dmi_memory_device_decode(dmi_entity_t *entity, dmi_version_t *plevel)
{
    ...
    data = dmi_cast(data, dmi_entity_data(entity, DMI_TYPE_MEMORY_DEVICE));
    if (data == nullptr)
        return nullptr;
